...
DWORD WINAPI clientSend(LPVOID lpParam) {
    ...
    // Simplified Conversion and Sending
    std::string sendBuffer; // Use std::string for dynamic UTF-8 conversion
    sendBuffer.resize((wcslen(buffer) + 1) * 4); // Reserve enough space
    int charsConverted = WideCharToMultiByte(CP_UTF8, 0, buffer, -1, &sendBuffer[0], sendBuffer.size(), NULL, NULL);

    if (charsConverted > 0) {
        if (send(server, sendBuffer.c_str(), charsConverted - 1, 0) == SOCKET_ERROR) { // -1 to exclude null terminator
...
DWORD WINAPI clientReceive(LPVOID lpParam) {
    ...
    while (true) {
        ...
        buffer[bytesReceived / sizeof(TCHAR)] = '\0'; // Null-terminate the received string   
        SendMessage(ListBox, LB_ADDSTRING, 0,  (LPARAM)buffer);// Add the received message to the list box
